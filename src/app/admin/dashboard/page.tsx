                            ...
                            <tbody>
                                {displayedOrders.map((order) => (
                                    <tr key={order.id}>
                                        ...
                                        <td className={styles.customerName}>{order.customer?.name || 'Guest'}</td>
                                        <td className={styles.orderTotal}>₦{order.total?.toLocaleString() || '0'}</td>
                                        <td>
                                            <span className={`${styles.statusBadge} ${styles[(order.status || 'pending').toLowerCase()]}`}>
                                                {order.status || 'pending'}
                                            </span>
                                        </td>
                                        <td className={styles.orderTime}>{order.date ? new Date(order.date).toLocaleDateString() : 'N/A'}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
